...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate BrowserGym multi-site config from local port range."
    )
    ...
    parser.add_argument(
        "--shared-model-path",
        default="artifacts/models/jaws_browsergym_shared_ppo_v2_browsergym_raw_obs.pt",
    )
    ...
    for port in range(args.start_port, args.end_port + 1):
        ...
        site: Dict[str, Any] = {
            ...
            # 중요:
            # Windows 환경에서 Python/Playwright가 localhost를 IPv6(::1)로 해석하면서
            # WinError 10053/10054가 발생할 수 있으므로 127.0.0.1을 사용한다.
